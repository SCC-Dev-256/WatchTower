...
"""helo_start_recording - Send a command to start Recording from a HELO, 
   Note that Recording may not actually start, if the device is not already properly configured.
   ...
   Example: $ python helo_start_recording.py http://192.168.0.2
   """
...
value = replicator_command_map["START_RECORDING"]
record_url = "{url}/config?action=set&paramid={p}&value={v}".format(url=device_url, p=param_id, v=value)
