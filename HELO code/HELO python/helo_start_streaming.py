...
"""helo_start_streaming - Send a command to start Streaming from a HELO, 
   Note that Streaming may not actually start, if the device is not already properly configured.
   ...
   Example: $ python helo_start_streaming.py http://192.168.0.2
   """
...
value = replicator_command_map["START_STREAMING"]
stream_url = "{url}/config?action=set&paramid={p}&value={v}".format(url=device_url, p=param_id, v=value)
