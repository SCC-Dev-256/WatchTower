...
###############################################################################
#
# helo_start_recording - bash shell script to initiate recording on a HELO, 
...
# 'main' start_record function
function start_record
{
	local DEVICE_URL=$1

	echo "Starting Record on HELO at ${DEVICE_URL}"

	local PARAM_ID="eParamID_ReplicatorCommand"
	local VALUE=1 	# Value for "Start Recording" Replicator command
...
# Call the start_record function
start_record "${DEVICE_URL}"
exit ?$
